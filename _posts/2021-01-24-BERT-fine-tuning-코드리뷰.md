...
## BERT Fine-Tuning이란?
   BERT는 대용량의 텍스트 코퍼스 단어 임베딩을 MLM과 NSP 방식을 통해 학습한 transformer 모델이다. MLM과 NSP task를 잘 수행하도록 학습된 BERT는 언어의 맥락적, 문법적 특징을 이해할 수 있다고 여겨진다. 이와 같이 사전학습된 모델의 파라미터를 조정하여 해결하고자 하는 타겟 task를 잘 수행하도록 재학습하는 과정이 fine-tuning 과정이다.<br/><br/> 이 글에서는 multi-class text classification을 잘하도록 BERT 파라미터를 fine-tuning하는 코드를 리뷰한다.
<br/>
